...
describe('POST /signup', () => {
  ...
  test('with password less than 8 characters should return 400', async () => {
    ...
    expect(responseBody.issues[0]?.message).toBe('Too small: expected string to have >=8 characters')
  })
  ...
  test.for([
    ...
    expect(response.status).toBe(400)
    expect(responseBody.issues.length).toBe(1)
    expect(responseBody.issues[0]?.message).toBe('Utilize only letters, numbers and hifens. Do not start/finish with hifen or have consecutive hifens')
  })
})
